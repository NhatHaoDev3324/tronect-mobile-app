...
export const tenantRegisterWithEmail = async (username: string, email: string, phone: string, password: string, otp: string) => {
    const request = await api.post("/api/tenant/register-by-email", { username, email, phone, password, otp });
    return request.data;
};
