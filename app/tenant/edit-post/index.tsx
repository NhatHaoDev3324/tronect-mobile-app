import { getPostRoomSharingById, updatePostRoomSharing } from "@/api/postRoomShareApi";
...
export default function EditPostScreen(props: ThemedViewProps) {
    ...
    const pickImages = async () => {
        ...
        const result = await ImagePicker.launchImageLibraryAsync({
            ...
            quality: 1,
            selectionLimit: MAX_IMAGES - displayImages.length,
        });
    ...
    useEffect(() => {
        const fetchData = async () => {
            if (!id) return;
            try {
                const res: PostInfoType = await getPostRoomSharingById(id);
                setTitle(res.title);
    ...
    const validateForm = (): FormErrors => {
        ...
        if (displayImages.length > MAX_IMAGES) {
            newErrors.images = `Chỉ được chọn tối đa ${MAX_IMAGES} ảnh`;
        }
                ...
                <View className="flex-col gap-4 border border-border rounded-xl p-4 mb-2">
                    ...
                    >
                        ...
                        <Text className="text-muted-foreground">
                            Chọn ảnh từ thiết bị ({displayImages.length}/{MAX_IMAGES})
                        </Text>
                ...
                <AuthPressable onAuthorizedPress={handleSubmit} style={{
                    ...
                    <View className="flex-row items-center gap-2">
                        ...
                        <Text className="text-white font-semibold">
                            {loading ? "Đang cập nhật bài đăng..." : "Cập nhật bài đăng"}
                        </Text>
                    </View>
                </AuthPressable>
            </ScrollView>
