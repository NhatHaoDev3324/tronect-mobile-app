import { SendLandlordOTPResetPass, VerifyOTPLandlord } from "@/api/authLandlordApi";
...
export default function ForgotPassword() {
  ...
  const onVerifyOtp = async (otp: string) => {
    ...
    setOtpLoading(true);
    try {
      const res = await VerifyOTPLandlord(mail, otp);
      ...
      if (res.status) {
        ...
        setOtpOpen(false);
        router.push({
          pathname: "/landlord/reset-password" as any,
          params: {
            id: res.data.id,
          },
        });
  ...
  const SendOtpResetPass = async () => {
    ...
    setLoading(true);
    try {
      const res = await SendLandlordOTPResetPass(mail);
    ...
    <View className="flex-1 bg-background">
        ...
        >
          <View className="flex flex-col gap-4 items-center justify-center">
            ...
            <Text className="text-center text-muted-foreground w-80">
              Nhập email đã đăng ký tài khoản Tronect với vai trò là Chủ trọ để nhận được mã OTP đặt lại mật khẩu.
            </Text>
          </View>
      ...
      <OtpModal
        ...
        onResend={async () => {
          ...
          try {
            setOtpLoading(true);
            await SendLandlordOTPResetPass(email);
