...
import { paymentExtendLink } from "@/api/payosApi";
...
export default function ChoosePackageScreen(props: ThemedViewProps) {
    ...
    const handlePayment = async () => {
        ...
        try {
            ...
            const res = await paymentExtendLink(
                resPayment.data.id,
                Number(calculateTotalPrice()),
                `Thanh toán gia hạn tin`,
                itemName,
                Number(calculateTotalPrice()),
                1,
    ...
    useEffect(() => {
        const loadTempPost = async () => {
            ...
            try {
                ...
                setPostType(data.post_type as "normal" | "vip");
            } catch (e) {
        ...
        <View className="flex-1" style={{ backgroundColor }}>
            <View style={{ paddingTop: insets.top + 12, backgroundColor: "#2baf90" }} className="flex-row items-center justify-between border-b border-border px-4 py-3">
                <View style={{ display: "flex", flexDirection: "row", alignItems: "center" }}>
                    ...
                    <Text className="text-xl font-bold text-white">Gia hạn tin đăng</Text>
                ...
                <View className="flex-1">
                    ...
                    >
                        <View className="flex-col gap-2 border border-border rounded-xl p-4">
                            <Text className="font-bold text-lg text-foreground">Chọn gói gia hạn tin đăng</Text>

                            <View className="flex-col">
                                <Text className="font-semibold text-foreground mb-1">Gói thời gian</Text>
                                <StatusDropdown
                                    data={[
                                        { label: `Gia hạn theo ngày ( ${Number(postPriceDay).toLocaleString()} đ/ngày )`, value: "day" },
                                        { label: `Gia hạn theo tuần ( ${Number(postPriceWeek).toLocaleString()} đ/tuần )`, value: "week" },
                                        { label: `Gia hạn theo tháng ( ${Number(postPriceMonth).toLocaleString()} đ/tháng )`, value: "month" },
                        ...
                        <View className="flex-col gap-2 border border-blue-500 bg-blue-50 dark:bg-blue-950 rounded-xl p-4 mb-8">
                            ...
                            <View className="flex-row items-center justify-between">
                                ...
                                <Text className="text-sm font-semibold text-foreground">
                                    {timeType === "day" ? `Gia hạn theo ngày ( ${Number(postPriceDay).toLocaleString()} đ/ngày )` : timeType === "week" ? `Gia hạn theo tuần ( ${Number(postPriceWeek).toLocaleString()} đ/tuần )` : `Gia hạn theo tháng ( ${Number(postPriceMonth).toLocaleString()} đ/tháng )`}
                                </Text>
                            </View>
                            <View className="flex-row items-center justify-between">
